...
def process_batch(batch, obs_horizon, action_horizon, device, flow_vqvae: Optional[torch.nn.Module] = None):
    ...
    if flow_vqvae is not None and "optical_flow" in batch:
        # flow = batch["optical_flow"][:, -action_horizon:].to(device)
        #FIXME：这里应该就是当前帧里面存储的t+action-horizon的flow,用8或者16
        # flow = batch["optical_flow"][:,action_horizon:].to(device)
        #FIXME
...
def eval_one_epoch(config, data_loader, device, model, flow_vqvae=None):
    model.eval()
...
def train_one_step(config, model, optimizer, scheduler, scaler, batch, device, flow_vqvae=None):
    model.train()
...
def maybe_evaluate(config, step, model, loader, device, action_normalizer=None, flow_vqvae=None):
    ...
    while step < config.num_steps:
        ...
        for batch in train_loader:
            ...
            loss, info = train_one_step(
                config, model, optimizer, scheduler, scaler, batch, device, flow_vqvae=flow_vqvae
            )
